...
class RewindButton(QPushButton):
    ...
    def paintEvent(self, event):
        ...
        # Draw white rewind icon with offset if pressed
        painter.setBrush(QColor("white"))
        painter.setPen(Qt.NoPen)
        
        # Adjusted rewind triangle for 30x30 size
        triangle = QPolygon([
            QPoint(9 + self.pressed_offset_x, 15 + self.pressed_offset_y),
            QPoint(22 + self.pressed_offset_x, 6 + self.pressed_offset_y),
            QPoint(22 + self.pressed_offset_x, 24 + self.pressed_offset_y)
        ])
        painter.drawPolygon(triangle)
        
        # Adjusted left bar
        painter.drawRect(6 + self.pressed_offset_x, 6 + self.pressed_offset_y, 3, 18)
